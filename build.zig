...
pub fn build(b: *std.Build) void {
    ...
    switch (os_tag) {
        .macos => {
            engine_mod.addObjectFile(b.path("deps/lib/macos/libglfw3.a"));
            engine_mod.linkSystemLibrary("c++", .{});
            engine_mod.linkFramework("Cocoa", .{});
            engine_mod.linkFramework("IOKit", .{});
            engine_mod.linkFramework("CoreVideo", .{});
            engine_mod.linkFramework("QuartzCore", .{});
            engine_mod.linkFramework("Metal", .{});
        },
        else => {},
    }
    engine_mod.addLibraryPath(deps_lib);
    engine_mod.linkSystemLibrary("wgpu_native", .{});
    engine_mod.addCSourceFile(.{
        .file = b.path("deps/src/glfw3webgpu.m"),
        .flags = &.{
            "-DGLFW_INCLUDE_NONE",
            "-DWEBGPU_BACKEND_WGPU",
    ...
    const gametest_mod = b.createModule(.{
        // `root_source_file` is the Zig "entry point" of the module. If a module
        // only contains e.g. external object files, you can make this `null`.
        // In this case the main source file is merely a path, however, in more
        // complicated build scripts, this could be a generated file.
    ...
    gametest_mod.addImport("mythopia", engine_mod);
